/**
 * /devo [descrição] [valor] [parcelas]
 * Exemplo: /devo celular 1500 12
 * Exemplo: /devo almoço 35
 * Cria registro(s) na coleção dividas com tipo="Devo"
 * Se parcelas > 1, gera um documento por mês a partir do mês atual
 */

module.exports = async (ctx, db) => {
  const text = ctx.message.text.replace('/devo', '').trim();

  if (!text) {
    return ctx.reply(
      '❌ Formato inválido.\n\nUse: /devo [descrição] [valor] [parcelas]\nExemplo: /devo celular 1500 12\nExemplo: /devo almoço 35'
    );
  }
  ...
  if (isNaN(valor) || valor <= 0) {
    return ctx.reply('❌ Valor inválido. Ex: /devo celular 1500 12');
  }
  ...
  try {
    ...
    for (let i = 0; i < parcelas; i++) {
      ...
      promises.push(db.collection('dividas').add({ tipo: 'Devo', data, descricao, valor: valorParcela, status }));
    }
